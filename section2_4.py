...
PinDown = 13
PinBuzzer = 18
Input_pins = [PinEcho,PinLeft,PinRight, PinUp,PinDown]
Output_pins = [PinTrigger,PinBuzzer]
...
def counter(channel):               # function to check whether the left button is pressed or not inorder to pause or restart the system
    global c, cali_start, cali_process
    if cali_start == 0:
        print("THE SYSTEM IS NOT MEASURING NOW AND CALIBRATION YET TO BE DONE")
    elif cali_process == 1:
        print("THE SYSTEM IS NOT MEASURING NOW AND PLEASE WAIT AS CALIBRATION IS GOING ON")
    else:
        c += 1
...
def calibration(channel):           # function to start auto-calibration if down button is pressed
    global alpha, c, cali_process, cali_start
    cali_start = 1
    t_echo = np.zeros(10)
    if c % 2 == 0:
        cali_process = 1
        ...
        for l in range(10):
            t_echo[l] = measurement(PinTrigger, PinEcho, c)
            time.sleep(0.5)
        GPIO.output(Output_pins[1], GPIO.LOW)
        average = t_echo.mean()
        print("The measured 10 values: ",t_echo)
        print("The mean of t_echo is: ",t_echo.mean())
        alpha = 100 / average
        cali_process = 0
    else:
        print("Please restart the system to start the calibration")
...
GPIO.add_event_detect(PinDown,GPIO.RISING,callback = calibration)
...
while True:
    if cali_start == 0:
        print("Press down button to start calibration and find alpha \n\n")
        time.sleep(1)
    elif cali_process == 1:
        print("Calibration going on. Please wait. \n")
        time.sleep(1)
    else:
        ...
        if pulse_duration > 0:
            ...
            segment_7SD.write_display()
            device.clear()
            scaled_value = alpha * pulse_duration
